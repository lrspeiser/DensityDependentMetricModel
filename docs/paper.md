...
Figure 2. Observed Gaia DR3 median stellar speeds with 16–84% band (gray) compared to our GR (baryons-only; blue dashed) and TFR tidal-band (red solid) predictions using the fitted parameters from the matched runs. This figure is generated by tools/plot_rotation_comparison.py and reflects the exact analysis data used in §7.
