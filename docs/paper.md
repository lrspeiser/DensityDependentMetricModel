...
- Solar-system sharper screening variant implemented: S_ρ(ρ)=exp[−(ρ/ρ_c)^{γ_exp}] (use --rho-screen exp in tools/fit_sparc_er_env.py). We will re-run Solar-System checks (LLR/DE440) and publish a comparison table (Table S3).
...
See docs/ED-SPARC.md for the refreshed table generated from images/sparc_evidence_triplet_summary.csv. Current coverage: 9 galaxies (8 with ER/GR, 9 with NFW entries). We include a supplementary table with ER hyperparameters (ρ_c, γ_exp, λ_max, T_0, σ_lnT, w_min) per galaxy (docs/ED-SPARC-params.md; generated by scripts/gen_ed_sparc_params_md.py).
