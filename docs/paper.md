...
![Milky Way rotation curve: Gaia vs GR vs TFR](../images/rotation_comparison_tidal_band.png?v=20250809-2)

Figure 2. Observed Gaia DR3 median stellar speeds with 16–84% band (gray) compared to our GR (baryons-only; blue dashed), ΛCDM/NFW (green dash-dot; when plotted, taken from the matched NFW evidence run), and TFR tidal-band (red solid) predictions using the fitted parameters from the matched runs. This figure is generated by tools/plot_rotation_comparison.py and reflects the exact analysis data used in §7.
...
Matched ΛCDM/NFW evidence (in progress). A matched NFW evidence run is underway with identical sampler controls to GR/TFR. Upon completion we will report ΔlogZ(TFR−NFW) alongside ΔlogZ(TFR−GR) and update Fig. 2 with the NFW curve explicitly sourced from the evidence run (never a plot-only fit). See docs/mw_triplet.md for the triad artifact and numbers.
