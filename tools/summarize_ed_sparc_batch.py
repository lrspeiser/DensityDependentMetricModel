...
def write_markdown_table(items: List[Dict[str, Any]]):
    ...
    headers = ['Galaxy','logZ_GR','logZ_NFW','logZ_ER','ΔlogZ(ER−GR)','ΔlogZ(ER−NFW)']
...
def write_summary(items: List[Dict[str, Any]]):
    ...
    lines.append(f"- ER preferred over GR (ΔlogZ>0): {n_pref_er_over_gr}/{n}")
    lines.append(f"- ER preferred over NFW (ΔlogZ>0): {n_pref_er_over_nfw}/{n}")
    lines.append("")
...
def main():
    ...
    for r in items[:5]:
        print(f"  {r['galaxy']}: ΔlogZ(ER−GR)={format_float(r['dER_GR'])}, ΔlogZ(ER−NFW)={format_float(r['dER_NFW'])}")
