#!/usr/bin/env python3
"""
fit_sparc_er_evidence.py - ER evidence for a SPARC galaxy using dynesty.
Non-invasive: mirrors style of GR/NFW evidence tools and writes a JSON sidecar.
"""
...
def main():
    ...
    # ER parameter bounds (uniform priors)
    ...
    # Params: [lambda_max, R0, sigma_lnR, w_min, ups_disk, ups_bul]
    lo = np.array([
    ...
    def loglike(theta):
        lam, R0, sLR, wmin, ups_d, ups_b = map(float, theta)
    ...
    dsampler = dynesty.DynamicNestedSampler(loglike, prior_transform, ndim=6, bound='multi', sample='rslice', rstate=rng)
    ...
    out_json = out_dir / f'sparc_er_evidence_{args.galaxy_id.lower()}.json'
    meta = {
        'galaxy_id': args.galaxy_id,
        'model': 'er_radial_log_normal',
