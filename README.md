...
> This version preserves your original opening sections and arguments while **adding the missing technical ingredients** reviewers will expect: the **explicit weak‑field formula actually used in the code and figures**, the **Solar‑System (Cassini) calculation** that is in the repository, and a clear statement of the **\(a_0\) fitting strategy** (per‑galaxy grid with optional hierarchical inference). We also elevate the important lensing and \(K_z/\Sigma_{1.1}\) milestones and mark what remains as **placeholders** to be filled with measured inputs or derivations. The orchestration/CLI and “pilot” lensing dials are moved to Methods/Supplement to keep the main text publishable.
