...
We compute the vertical force K_z(R_0, z) using the same Milky Way baryonic mass model (Miyamoto–Nagai disks + Hernquist bulge) used for the radial curve and infer Σ_1.1 via the Oort‑limit approximation K_z ≈ 2πG Σ(<|z|). In addition to the baryons‑only curve, we also show a DGG‑scaled approximation (using the local D factor at R0 from the RAR‑plateau fit) for quick comparison. A follow‑up will add the full 3D effective “phantom” component and compare against Bovy & Rix / McMillan bands.

![Milky Way Kz and Σ_1.1 (baryons and DGG‑scaled approx)](images/next_steps/enhanced_20250805_115400/mw_kz_sigma.png)

Source data: results/next_steps/enhanced_20250805_115400/mw_kz_sigma.csv (includes component K_z and Σ_inferred columns).
...
#### SPARC gold-sample panel (RAR‑plateau vs GR vs observations)
![SPARC gold overlays panel](images/next_steps/enhanced_20250805_115400/sparc_panel_gold.png)
*Representative overlays for the gold sample (M31, NGC 3198, NGC 2403, NGC 2841, NGC 5055). Black points with error bars are observed rotation speeds; blue dashed curves are GR baryons; red curves are RAR‑plateau with per‑galaxy a0 fit. See results/next_steps/enhanced_20250805_115400/sparc_a0_summary.csv for the a0 values and Δχ².*
...
If you provide docs/lcdm_rar_band.csv (format/spec in docs/lcdm_rar_band.md), the ΛCDM band will appear in the panel for direct visual contrast at low g_bar, with discriminants called out in the figure caption in the paper text.
