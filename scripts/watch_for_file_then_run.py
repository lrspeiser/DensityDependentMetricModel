...
def main():
    ...
    # Option A: pass command via --command python script.py --arg ... (may be tricky with extra --args)
    ap.add_argument('--command', nargs='+', help='Command to execute when file is ready')
    # Option B: use a standard "--" separator and provide the command as the remainder
    ap.add_argument('remainder', nargs=argparse.REMAINDER, help='Command to execute (use after -- separator)')
    ...
    if args.command:
        ...
        # remainder typically starts with a leading '--' when provided; strip it
        rem = [x for x in args.remainder if x != '--']
        if rem:
            cmd = rem
    if not cmd:
        print('Error: no command provided. Use --command ... or place the command after a -- separator.')
        sys.exit(2)
    ...
    while True:
        ...
        if target.exists():
            ...
            if last_mtime is None or mtime != last_mtime:
                ...
                print(f'File detected and stable; running: {" ".join(cmd)}')
